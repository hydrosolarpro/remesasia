-- ─────────────────────────────────────────────────────────────
-- 0079 — Arregla eliminar_operador_peru: fallaba con
-- "violates foreign key constraint invitaciones_usado_por_fkey" porque la
-- cuenta del propio operador (y las de sus miembros/VE) normalmente se
-- crearon canjeando una invitación tipo 'operador_peru' -- esa fila de
-- `invitaciones` queda con `usado_por` apuntando a esa cuenta, y el
-- borrado original solo cubría `usado_por` de los CLIENTES, no del
-- operador/miembros/VE.
-- ─────────────────────────────────────────────────────────────
...
begin
  ...
  --    negocio -- incluidas las que el operador, sus miembros o sus VE ya
  --    canjearon para crear su propia cuenta (`usado_por`), no solo las
  --    que ellos crearon (`creado_por`) o las de sus clientes.
  ...
  delete from invitaciones
    where negocio_operador_peru_id = p_operador_id
       ...
       or usado_por = p_operador_id
       or usado_por = any(v_miembro_usuario_ids)
       or usado_por = any(v_ve_usuario_ids)
