-- ─────────────────────────────────────────────────────────────
-- 0080 — Arregla eliminar_operador_peru: seguía fallando con
-- "violates foreign key constraint operador_peru_miembro_usuario_id_fkey"
-- porque la versión anterior solo desvinculaba operador_peru_miembro.usuario_id
-- / operador_venezuela_perfil.usuario_id DENTRO del propio negocio
-- (`where operador_peru_id = p_operador_id`) -- pero una cuenta puede
-- haber quedado referenciada como `usuario_id` en una fila de equipo de
-- OTRO negocio (p. ej. si esa persona tuvo antes otro rol/negocio). Ahora
-- se desvincula cualquier fila, de cualquier negocio, que apunte a una de
-- las cuentas que se van a borrar -- el mismo criterio se aplica a
-- `invitaciones`, `cambios_plan_pendientes` y `prospectos`.
-- ─────────────────────────────────────────────────────────────
...
begin
  ...
  -- 5) Red de seguridad: cualquier otra fila (de cualquier negocio) que
  --    apunte a una cuenta que se va a borrar se desvincula en vez de
  --    bloquear el borrado.
